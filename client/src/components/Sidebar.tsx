...
function Sidebar({ activePage, setActivePage, onOpenPalette }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className="w-64 min-h-screen bg-gray-900 border-r border-gray-800  flex-col p-6 pt-14 font-inter hidden md:flex">
        ...
        <h2 className="text-lg font-bold text-indigo-400 mb-4">Dashboard</h2>

        {onOpenPalette && (
          <button
            onClick={onOpenPalette}
            className="w-full mb-4 flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 transition-colors text-sm cursor-pointer"
          >
            <Search size={14} />
            <span className="flex-1 text-left text-xs">Search...</span>
            <kbd className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-[10px] font-mono text-gray-500">Ctrl K</kbd>
          </button>
        )}
