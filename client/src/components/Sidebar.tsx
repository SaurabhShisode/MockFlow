import { Home, PlusCircle, Boxes, List, Settings, Menu, X } from 'lucide-react';
...
function Sidebar({ activePage, setActivePage }: SidebarProps) {
      ...
      <nav className="space-y-2 flex-1">
        {menu.map(item => (
          <button
            ...
            className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-3 transition-all duration-200 active:scale-95 ${
              activePage === item.id
  ...
  return (
    <>
      ...
      <aside className="w-64 min-h-screen bg-gray-900 border-r border-gray-800 flex flex-col p-6 font-inter pt-14 hidden md:flex">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="relative w-64 min-h-screen bg-gray-900 border-r border-gray-800 flex flex-col p-6 font-inter pt-14 z-50">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-semibold text-white font-grotesk">MockFlow</h1>
              </div>
              <button
                onClick={() => setMobileOpen(false)}
